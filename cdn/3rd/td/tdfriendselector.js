...
var TDFriendSelector = (function(module, $) {
	...
	setFriends = function(input) {
        ...
        {
            if (input[i].is_app_user == '1')
            {
                bucket1.push (input[i]);
            }
            else
            {
                bucket0.push (input[i]);
            }
        }
        ...
        bucket0 = this.shuffleArray (bucket0);
        bucket1 = this.shuffleArray (bucket1);
        input = Array.prototype.concat (bucket1, bucket0);

		for (i = 0, len = input.length; i < len; i += 1) {
			input[i].upperCaseName = input[i].name.toUpperCase();
		}
        // input = this.shuffleArray (input);
	...
	newInstance = function(options) {
		...
		showFriendSelector = function(callback) {
			...
			if (!$friends) {
				...
				for (i = 0, len = friends.length; i < len; i += 1) {
					if ($.inArray(friends[i].uid, selectedFriendIds) !== -1) {
						$($friends[i]).addClass(settings.friendSelectedClass);
					}
					if ($.inArray(friends[i].uid, disabledFriendIds) !== -1) {
	...
	buildFriendSelector = function(callback) {
		...
		FB.getLoginStatus(function(response) {
			if (response.status === 'connected') {
				...
				FB.api('/fql?q=' + encodeURIComponent ('SELECT uid, name, is_app_user FROM user WHERE uid IN (SELECT uid2 FROM friend WHERE uid1 = me())'), function(response) {
					if (response.data) {
		...
		buildFriendMarkup = function(friend) {
			return '<a href="#" class="TDFriendSelector_friend TDFriendSelector_clearfix" data-id="' + friend.uid + '">' +
					'<img src="//graph.facebook.com/' + friend.uid + '/picture?type=square" width="50" height="50" alt="' + friend.name + '" class="TDFriendSelector_friendAvatar" />' +
